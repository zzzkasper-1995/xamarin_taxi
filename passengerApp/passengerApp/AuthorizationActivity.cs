...
{
    [Activity(Label = "YouTaxi водитель", MainLauncher = true, Icon = "@drawable/icon")]
                ...
                if (mSharedPrefs.GetString("key", "") != "") //если есть сохраненный логин и пароль то
                    if (mSharedPrefs.GetString("login", "") != "")
                    {
                        try
                        {
                            ...
                            if (ans.cod == "131") MessageBox("Ошибка", "Пользователь с номером " + user.number + " заблокирован", "Ок");
                            else
                            if (ans.cod != "130")
                                if (ans.cod == "17")
                                {
                                ...
                                else if (ans.cod == "132" || ans.cod == "133")//открываем окно регистрации
                                {
                                    Intent intent = new Intent(this, typeof(RegActivity));
                                    intent.SetFlags(ActivityFlags.NoHistory);
                                    StartActivity(intent);
                ...
                if (pas.Text.Length == 5)
                {
                    ...
                    try
                    {
                        ...
                        if (isConn)
                        {
                            ...
                            else ans = ConWithServ.getNewPas(num.Text, user.cod);//если старый
                            ...
                            if (ans.cod == "3" || ans.cod == "16")
                            {
                                ...
                                user.name = "";
                                user.surname = "";
                                ans = ConWithServ.hello(user.number, user.cod);
                            ...
                            {
                                MessageBox("Ошибка", "Неверный логин или пароль", "Повторить");
                            }
                        }
                        else
                        {
